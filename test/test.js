...
describe('select rows', function () {
  it('should return data as array', async function () {
...
describe('where', function () {
  it('when {where} was set should return correct row(s)', async function () {
    ...
    await mysql
      .table('admins')
      .where({ id: ['between', [5, 10]], name: 'admin' })
      .find();
    ...
    await mysql
      .table('admins')
      .where({ id: ['between', [5, 10]], name: 'admin', _logic: 'OR' })
      .find();
...
describe('limit', function () {
  it('when {limit} was set should only return ${limit} items', async function () {
    const data = await mysql.table('article_categories').limit(2).select();
...
describe('page', function () {
  it('when {page} was set should only return ${offset}, ${limit} items', async function () {
    const data = await mysql.table('article_categories').page(2, 2).select();
    expect(data).to.have.lengthOf(2);
...
describe('order', function () {
  it('when {order} was set should only return correct order items', async function () {
    const data1 = await mysql.table('article_categories').order('id desc').select();
    expect(data1[0].id).to.be.above(data1[1].id);
    const data2 = await mysql.table('article_categories').order(['id desc', 'name asc']).select();
...
describe('update', function () {
  it('when {update} operate should update the correct rows', async function () {
    ...
    const data3 = await mysql.table('article_categories').where({ id: 5 }).update({ name: '测试update' });
